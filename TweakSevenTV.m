...
static S7TVChatMessage * _Nullable s7tv_parseChatMessage(NSString *ircLine) {
    S7TVChatMessage *message = s7tv_parsePRIVMSG(
        ircLine, s7tv_emoteProviders(),
        ^S7TVChannelPointRewardInfo *(NSString *messageID) {
            return s7tv_automaticRewardInfoForIRCMessageID(messageID);
        });
    return message ?: s7tv_parseUSERNOTICE(ircLine, s7tv_emoteProviders());
}
...
static void s7tv_fetchRecentHistory(NSString *channel, NSUInteger generation) {
    ...
    [[[NSURLSession sharedSession] dataTaskWithRequest:request
        ...
        for (id value in rawMessages) {
            ...
            S7TVChatMessage *message = s7tv_parseChatMessage(ircLine);
            if (message) {
            ...
            if (!error && message) {
                ...
                if (textToProcess) {
                    ...
                    for (NSString *rawLine in ircLines) {
                        ...
                        S7TVChatMessage *chatMsg = s7tv_parseChatMessage(ircLine);
                        if (!chatMsg) continue;
